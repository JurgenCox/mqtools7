...
namespace PluginInterop{
	...
	public static class Utils{
		...
		public static IData[] ReadSupplementaryData(string[] suppFiles, DataType[] supplDataTypes,
			ProcessInfo processInfo, IData inData){
			...
			for (int i = 0; i < numSupplTables; i++){
				switch (supplDataTypes[i]){
					case DataType.Matrix:
						IMatrixData mdata = (IMatrixData) inData.CreateNewInstance(DataType.Matrix);
						PerseusUtil.ReadMatrixFromFile(mdata, processInfo, suppFiles[i], '\t');
						supplData[i] = mdata;
						break;
					case DataType.Network:
						INetworkData ndata = (INetworkData) inData.CreateNewInstance(DataType.Network);
