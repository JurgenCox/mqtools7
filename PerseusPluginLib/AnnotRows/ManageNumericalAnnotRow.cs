...
namespace PerseusPluginLib.AnnotRows{
	public class ManageNumericalAnnotRow : IMatrixProcessing{
		...
		public string Url =>
            "https://cox-labs.github.io/coxdocs/managenumericalannotrow.html";
		...
		private static string ProcessDataReadFromFile(IDataWithAnnotationRows mdata, Parameters param){
			...
			foreach (string numColName in numColNames){
				AddNumericRow(numColName, filename, mdata, map);
			}
			foreach (string catColName in catColNames){
				ManageCategoricalAnnotRow.AddCategoryRow(catColName, filename, mdata, map);
			}
			return null;
		}
		internal static void AddNumericRow(string numColName, string filename, IDataWithAnnotationRows mdata,
			Dictionary<string, int> map){
			string[] groupCol = TabSep.GetColumn(numColName, filename, '\t');
			...
			if (numColName.ToLower().EndsWith("[n]")){
				numColName = numColName.Substring(0, numColName.Length - 3);
				numColName = numColName.Trim();
			}
			mdata.AddNumericRow(numColName, numColName, newCol);
		}
		...
		public Parameters GetParameters(IMatrixData mdata, ref string errorString){
			SingleChoiceWithSubParams scwsp = new SingleChoiceWithSubParams("Action"){
				...
				SubParams = new[]{
					GetCreateParameters(mdata), GetEditParameters(mdata), GetRenameParameters(mdata),
					GetDeleteParameters(mdata), GetWriteTemplateFileParameters(mdata), GetReadFromFileParameters(mdata)
				},
				ParamNameWidth = 136,
				TotalWidth = 731
			};
		...
		public Parameters GetWriteTemplateFileParameters(IMatrixData mdata){
			List<Parameter> par = new List<Parameter>{
				new FileParam("Output file", "NumericalRows.txt"){
					Filter = "Tab separated file (*.txt)|*.txt", Save = true
				}
			};
			return new Parameters(par);
		}
