...
namespace PerseusPluginLib.Mods{
	public class KinaseSubstrateRelations : IMatrixProcessing{
		...
		public string Url
			=>
                "https://cox-labs.github.io/coxdocs/kinasesubstraterelations.html";
		...
		public void ProcessData(IMatrixData mdata, Parameters param, ref IMatrixData[] supplTables,
			...
			for (int i = 0; i < kinaseNameColumn.Length; i++){
				...
				foreach (string ux in uprot[i]){
					if (substrateProperties.ContainsKey(ux)){
						...
						foreach (Tuple<string, string, string> property in properties){
							...
							if (AddKnownSites.Contains(win1,
								    AddKnownSites.TransformIl(w.ToUpper().Substring(1, w.Length - 2)))){
				...
				kinaseNameColumn[i] =
					kinaseNamesHits.Count > 0 ? StringUtils.Concat(";", kinaseNamesHits.ToArray()) : "";
