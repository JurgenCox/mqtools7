...
namespace PerseusPluginLib.Norm{
	public class ZScore : IMatrixProcessing{
		...
		public string Url =>
            "https://cox-labs.github.io/coxdocs/zscore.html";
		...
		public void ProcessData(IMatrixData mdata, Parameters param, ref IMatrixData[] supplTables,
			...
			if (groupInd < 0){
				...
				ZscoreGroups(mdata, catRow, processInfo.NumThreads, report, median, groupVals, out double[][] means,
					out double[][] stddevs);
				if (report){
		...
		private static void ZscoreGroups(IMatrixData data, IList<string[]> catRow, int nthreads, bool report,
			bool median,
		...
		private static void ZscoreGroup(IMatrixData data, IList<int> inds, int nthreads, bool report, bool median,
			...
			new ThreadDistributor(nthreads, data.RowCount, i => Calc3(i, data, inds, doubles, stddevs1, report, median))
				.Start();
		}
		...
		private static void Zscore(bool rows, IMatrixData data, int nthreads, bool report, bool median,
			out double[] means,
			out double[] stddevs){
			means = null;
			stddevs = null;
			if (rows){
				...
				double[] doubles = means;
				double[] stddevs1 = stddevs;
				new ThreadDistributor(nthreads, data.RowCount, i => Calc1(i, data, doubles, stddevs1, report, median))
					.Start();
			} else{
				if (report){
				...
				double[] doubles = means;
				double[] stddevs1 = stddevs;
				new ThreadDistributor(nthreads, data.ColumnCount,
					j => Calc2(j, data, doubles, stddevs1, report, median)).Start();
			}
		}
