...
namespace PerseusPluginLib.Join{
	public class MatchingRowsByName : IMatrixMultiProcessing{
		...
		public Parameters GetParameters(IMatrixData[] inputData, ref string errString){
			...
			List<string> matchColumnNames =
				matrixData1.StringColumnNames.Concat(matrixData1.NumericColumnNames).ToList();
			List<string> matchColumnNames2 =
				matrixData2.StringColumnNames.Concat(matrixData2.NumericColumnNames).ToList();
			return new Parameters(CreateMatchParameters(matchColumnNames, matchColumnNames2).Concat(new Parameter[]{
		...
		public static Parameter[] CreateMatchParameters(List<string> matchColumnNames, List<string> matchColumnNames2,
			string tryFind = "uniprot"){
			int FindUniprot(List<string> names) => Math.Max(0, names.FindIndex(col => col.ToLower().Contains(tryFind)));
			return new Parameter[]{
				new SingleChoiceParam("Matching column in table 1"){
				...
				new BoolWithSubParams("Use additional column pair"){
					SubParamsTrue = new Parameters(
						new SingleChoiceParam("Additional column in table 1"){
							Values = matchColumnNames,
							Value = FindUniprot(matchColumnNames),
				...
				new BoolParam(ignoreCaseLabel, true){Help = "Ignore the capitalization of the string while matching"},
			};
		}
		...
		public IMatrixData ProcessData(IMatrixData[] inputData, Parameters parameters, ref IMatrixData[] supplTables,
			...
			(int[][] indexMap, int[] unmappedRightIndices) =
				PerseusUtils.GetIndexMap(mdata1, mdata2, matching.first, matching.second, matching.ignoreCase);
		...
		private static void UpdateIdColumns(IMatrixData result, IMatrixData right, int originalLength,
			...
			string[] idColumn = GetColumn(result, first.left);
			string[] rightIdColumn = GetColumn(right, first.right);
			int i = 0;
			foreach (int row in unmappedRightIndices){
			...
			if (first.left < result.StringColumnCount){
				...
				result.NumericColumns[first.left - result.StringColumnCount] =
					idColumn.Select(Convert.ToDouble).ToArray();
			}
			if (second.HasValue){
				string[] idColumn2 = GetColumn(result, second.Value.left);
				string[] rightIdColumn2 = GetColumn(right, second.Value.right);
				int j = 0;
				foreach (int row in unmappedRightIndices){
				...
				if (second.Value.left < result.StringColumnCount){
					...
					result.NumericColumns[second.Value.left - result.StringColumnCount] =
						idColumn.Select(Convert.ToDouble).ToArray();
				}
			}
		}
		public static ((int[] copy, int combine) main, int[] text, (int[] copy, int combine) numeric, int[] category)
			ParseCopyParameters(Parameters parameters){
			int[] exColInds = parameters.GetParam<int[]>("Copy main columns").Value;
			int combineMain = parameters.GetParam<int>("Combine copied main values").Value;
			int[] copyNumericalColumns = parameters.GetParam<int[]>("Copy numerical columns").Value;
			int combineNumerical = parameters.GetParam<int>("Combine copied numerical values").Value;
			int[] copyCatColumns = parameters.GetParam<int[]>("Copy categorical columns").Value;
			int[] copyTextColumns = parameters.GetParam<int[]>("Copy text columns").Value;
			return ((exColInds, combineMain), copyTextColumns, (copyNumericalColumns, combineNumerical),
				copyCatColumns);
		}
		public static ((int m1, int m2) first, (int m1, int m2)? second, bool outer, bool ignoreCase)
			ParseMatchingColumns(Parameters parameters){
			int Value(Parameters param, string name) => param.GetParam<int>(name).Value;
			(int, int) first = (Value(parameters, "Matching column in table 1"),
				Value(parameters, "Matching column in table 2"));
			...
			bool outer = parameters.GetParam<int>("Join style").Value == 1; // 0 left, 1 outer
			bool ignoreCase = parameters.GetParam<bool>(ignoreCaseLabel).Value;
		...
		private static void AddIndicator(IDataWithAnnotationColumns result, IData mdata2, int[][] indexMap){
			...
			string[] noIndicator = new string[0];
		...
		private static void AddMainColumns(IMatrixData result, IMatrixData mdata2, IList<int[]> indexMap,
			int[] exColInds, Func<double[], double> avExpression){
			if (avExpression != null){
				(string name, double[] values, double[] quality, bool[] isImputed)[] columns = exColInds.Select(i => {
					string name = mdata2.ColumnNames[i];
					double[] values = mdata2.Values.GetColumn(i).Unpack();
					double[] quality = mdata2.HasQuality ? mdata2.Quality.GetColumn(i).Unpack() : null;
					bool[] isImputed = mdata2.IsImputed.IsInitialized() ? mdata2.IsImputed.GetColumn(i) : null;
					return (name, values, quality, isImputed);
				}).ToArray();
				AddMainColumns(result, indexMap, columns, avExpression);
			} else{
				(string name, double[] values)[] columns = exColInds.Select(i => {
					string name = mdata2.ColumnNames[i];
					double[] values = mdata2.Values.GetColumn(i).Unpack();
					return (name, values);
				}).ToArray();
		...
		private static void AddMainColumns(IMatrixData result, IList<int[]> indexMap,
			...
			if (columns.Length > 0){
				...
				for (int i = 0; i < columns.Length; i++){
					(string name, double[] oldValues, double[] quality, bool[] isImputed) = columns[i];
		...
		private static void AddNumericColumns(IDataWithAnnotationColumns result, IDataWithAnnotationColumns mdata2,
			IList<int[]> indexMap, int[] copyNumericalColumns, Func<double[], double> avNumerical){
			(string, double[])[] columns = copyNumericalColumns
				.Select(i => (mdata2.NumericColumnNames[i], mdata2.NumericColumns[i])).ToArray();
			if (avNumerical != null){
		...
		private static void AddNumericColumns(IDataWithAnnotationColumns result, IList<int[]> indexMap,
			(string name, double[] values)[] columns, Func<double[], double> avNumerical){
		...
		private static void AddMultiNumericColumns(IDataWithAnnotationColumns result, IList<int[]> indexMap,
			(string name, double[] values)[] numericalColumns){
		...
		private static void AddCategoricalColumns(IDataWithAnnotationColumns mdata1, IDataWithAnnotationColumns mdata2,
			IList<int[]> indexMap, int[] copyCatColumns){
		...
		private static void AddStringColumns(IDataWithAnnotationColumns mdata1, IDataWithAnnotationColumns mdata2,
			IList<int[]> indexMap, int[] copyTextColumns){
		...
		private static void SetAnnotationRows(IDataWithAnnotationRows result, IDataWithAnnotationRows mdata1,
			IDataWithAnnotationRows mdata2, int[] exColInds){
			...
			int exColCount = exColInds.Length;
		...
		private static void AddMainColumns(IMatrixData data, ICollection<string> names, double[,] vals, double[,] qual,
			bool[,] imp){
			int n = data.RowCount;
