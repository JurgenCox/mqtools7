	...
	public static class DataWithAnnotationColumnsExtensions{
		...
		public static void AddEmptyRows(this IDataWithAnnotationColumns mdata, int length){
			for (int i = 0; i < mdata.StringColumnCount; i++){
				mdata.StringColumns[i] =
					mdata.StringColumns[i].Concat(Enumerable.Repeat(String.Empty, length)).ToArray();
			}
			for (int i = 0; i < mdata.NumericColumnCount; i++){
				mdata.NumericColumns[i] =
					mdata.NumericColumns[i].Concat(Enumerable.Repeat(Double.NaN, length)).ToArray();
			}
			for (int i = 0; i < mdata.MultiNumericColumnCount; i++){
				mdata.MultiNumericColumns[i] = mdata.MultiNumericColumns[i]
					.Concat(Enumerable.Range(0, length).Select(_ => new double[0])).ToArray();
			}
			for (int i = 0; i < mdata.CategoryColumnCount; i++){
				mdata.SetCategoryColumnAt(
					mdata.GetCategoryColumnAt(i).Concat(Enumerable.Repeat(new string[0], length)).ToArray(), i);
			}
		}
		/// <summary>
		...
		private static void ExtendMainColumns(this IMatrixData mdata, int length){
			...
			var quality = hasQuality
				? new float[mdata.Quality.RowCount + length, mdata.Quality.ColumnCount]
				: new float[0, 0];
			var hasImputation = mdata.IsImputed.IsInitialized();
			var isImputed = hasImputation
				? new bool[mdata.IsImputed.RowCount + length, mdata.IsImputed.ColumnCount]
				: new bool[0, 0];
