...
using MqUtil.Num.Test.Univariate;
...
namespace PerseusPluginLib.Utils{
	public static class TwoSampleTestUtil{
		...
		public static Parameters GetParametersImpl(IMatrixData mdata, ref string errorString, bool isVolcano,
			...
			return new Parameters(GetGroupingParam(mdata), GetTestParam(allTests, allNames),
				GetFilterValidValuesParam(),
				...
				new StringParam("Suffix"){
					Help =
						"This suffix will be attached to newly generated columns. That way columns from multiple runs of the " +
		...
		private static BoolWithSubParams GetCombinedScoreParam(){
			return new BoolWithSubParams("Calculate combined score"){
				...
				Help =
					"In case multiple two sample tests are performed, the combined score helps to define a global set of " +
		...
		private static Parameter GetSecondGroupParameter(IList<string> vals){
			return new SingleChoiceWithSubParams("Second groups mode"){
					...
					new[]{
						new Parameters(
							new MultiChoiceParam("Second group (left)"){
								...
								Repeats = true,
								Help =
									"The 'left' groups for the two sample tests. The number of selected items must match the number of " +
				...
				Help =
					"Specify here how the 'left' groups of the two sample tests are specified. Possible ways are to " +
				...
				SubParamsTrue =
					new Parameters(
						...
						new SingleChoiceParam("Min. number mode", 0){
							Values = new[]{
								"In total", "In both groups", "In at least one group", "In the first group",
								"In the second group"
							},
		...
		public static void ReadParams(bool isVolcano, IMatrixData mdata, Parameters param, ProcessInfo processInfo,
			out int groupRowInd, out int[] firstGroupInds, out int[] secondGroupInds,
			out SecondGroupMode secondGroupMode,
			...
			out bool filterValidValues, out int minNumValidValues, out int minNumValidValuesMode,
			out int minPercValidValues,
			...
			if (secondGroupMode != SecondGroupMode.Complement && firstGroupInds.Length != secondGroupInds.Length){
				processInfo.ErrString =
					"Please specify the same number of groups in the 'First group' and 'Second group' boxes.";
				return;
			}
			...
			ParameterWithSubParams<bool> combinedScoreParam =
				param.GetParamWithSubParams<bool>("Calculate combined score");
		...
		public static double[] Process(IList<int> firstGroupInds, IList<int> secondGroupInds,
			SecondGroupMode secondGroupMode, IList<string> groupNames, IMatrixData mdata, int groupInd,
			...
			string[] pvals1Name, string[] fdrs1Name, string[] diffs1Name, string[] statCol1Name,
			string[] significant1Name,
			...
			for (int itest = 0; itest < ntests; itest++){
				...
				string secondGroup = secondGroupMode == SecondGroupMode.Complement
					? null
					: groupNames[secondGroupInds[itest]];
				string err = null;
				PerformSingleTest(firstGroup, secondGroup, secondGroupMode, mdata, groupInd, ref err, log, threshold,
					truncation,
					nrand, preserveGroupInd, test, test1, side, s0, filterValidValues, minNumValidValues,
					minNumValidValuesMode,
					minPercValidValues, minPercValidValuesMode, out pvalsS0[itest], indMap, qval, plotNames, pvals1Name,
					fdrs1Name,
					diffs1Name, statCol1Name, significant1Name, pvals1, fdrs1, diffs1, statCol1, significant1, itest,
					sigCol,
		...
		private static void PerformSingleTest(string firstGroup, string secondGroup, SecondGroupMode secondGroupMode,
			IMatrixData mdata, int groupInd, ref string err, bool log, double threshold, TestTruncation truncation,
			int nrand,
			int preserveGroupInd, TwoSampleTest test, OneSampleTest test1, TestSide side, double s0,
			bool filterValidValues,
			...
			string[] fdrs1Name, string[] diffs1Name, string[] statCol1Name, string[] significant1Name,
			double[][] pvals1,
			double[][] fdrs1, double[][] diffs1, double[][] statCol1, string[][][] significant1, int ind,
			List<string>[] sigCol,
			...
			TwoSamplesTest1(colInds1, colInds2, truncation, threshold, test, test1, side, log, mdata, s0, nrand,
				colIndsPreserve1, colIndsPreserve2, suffix, mainSuffix, filterValidValues, minNumValidValues,
				minNumValidValuesMode,
				minPercValidValues, minPercValidValuesMode, out pvalsS0, randomized, addQval, plotNames, pvals1Name,
				fdrs1Name,
		...
		private static void TwoSamplesTest1(int[] colInd1, int[] colInd2, TestTruncation truncation, double threshold,
			...
			List<int[]> colIndsPreserve1, List<int[]> colIndsPreserve2, string suffix, string mainSuffix,
			bool filterValidValues,
			int minNumValidValues, int minNumValidValuesMode, int minPercValidValues, int minPercValidValuesMode,
			out double[] pvalsS0, bool randomized, bool addQval, IList<string> plotNames, IList<string> pvals1Name,
			IList<string> fdrs1Name, IList<string> diffs1Name, IList<string> statCol1Name,
			IList<string> significant1Name,
			...
			for (int i = 0; i < data.RowCount; i++){
				...
				if (valid){
					CalcTest(test, test1, paired, vals1, vals2, s0, side, out diffs[i], out pvals[i], out pvalsS0[i],
						out statCol[i]);
				} else{
					pvals[i] = double.NaN;
			...
			if (!randomized){
				...
				switch (truncation){
					...
					case TestTruncation.PermutationBased:
						double[] fdrsX;
						string[][] significantX = CalcPermutationBasedFdr(pvalsS0.SubArray(validRows), nrand, data,
							test,
							test1, side, colInd1, colInd2, s0, threshold, colIndsPreserve1, colIndsPreserve2, validRows,
							out fdrsX, paired);
		...
		private static void CalcTest(TwoSampleTest test, OneSampleTest test1, bool paired, double[] vals1,
			double[] vals2,
			...
			double rightS0;
			if (paired){
				test1.Test(ArrayUtils.ExtractValidValues(Diff(vals1, vals2)), 0, out _, out statS0, out both, out left,
					out right,
					out diff, s0, out bothS0, out leftS0, out rightS0);
			} else{
				test.Test(vals1, vals2, out _, out _, out _, out statS0, out both, out left, out right, out diff, s0,
					out bothS0,
		...
		private static bool IsValidRow(bool filterValidValues, int minNumValidValues, int minNumValidValuesMode,
			int minPercValidValues, int minPercValidValuesMode, int i, ICollection<int> colInd1,
			ICollection<int> colInd2,
			IMatrixData data){
			if (!filterValidValues){
				return true;
			}
		...
		private static bool PercValid(int v1, int v2, int t1, int t2, int minPercValidValues,
			int minPercValidValuesMode){
			switch (minPercValidValuesMode){
				case 0: return Math.Round((v1 + v2) * 100f / (t1 + t2)) >= minPercValidValues;
				case 1:
					return Math.Round(v1 * 100f / t1) >= minPercValidValues &&
					       Math.Round(v2 * 100f / t2) >= minPercValidValues;
				case 2:
					return Math.Round(v1 * 100f / t1) >= minPercValidValues ||
					       Math.Round(v2 * 100f / t2) >= minPercValidValues;
		...
		private static void BalancedPermutationsSubgroups(List<int[]> preserve1, List<int[]> preserve2,
			out int[] inds1Out,
			out int[] inds2Out, Random2 r2){
			PermBasedFdrUtil.BalancedPermutationsSubgroups(new[]{preserve1.ToArray(), preserve2.ToArray()},
				out int[][] out1,
				r2);
			inds1Out = out1[0];
			inds2Out = out1[1];
		...
		private static string[][] CalcPermutationBasedFdr(IList<double> pvalsS0, int nperm, IMatrixData data,
			TwoSampleTest test, OneSampleTest test1, TestSide side, int[] colInd1, int[] colInd2, double s0,
			double threshold,
			List<int[]> colIndsPreserve1, List<int[]> colIndsPreserve2, List<int> validRows, out double[] fdrs,
			bool paired){
			...
			for (int p = 0; p < nperm; p++){
				...
				if (!paired){
					if (colIndsPreserve1 != null){
						BalancedPermutationsSubgroups(colIndsPreserve1, colIndsPreserve2, out colInd1P, out colInd2P,
							r2);
					} else{
				...
				foreach (int row in validRows){
					...
					CalcTest(test, test1, paired, vals1, vals2, s0, side, out double _, out double _, out double p1,
						out double _);
					pq.Add(p1);
					indices.Add(-1);
				}
