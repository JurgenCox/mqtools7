...
namespace PerseusPluginLib.Basic{
	public class SummaryStatisticsRows : IMatrixProcessing{
		...
		internal static Tuple<string, Func<IList<double>, double>, string>[] procs ={
			...
			new Tuple<string, Func<IList<double>, double>, string>("Tukey biweight", TukeyBiweightCalc.TukeyBiweight,
				""),
			new Tuple<string, Func<IList<double>, double>, string>("Standard deviation", ArrayUtils.StandardDeviation,
				""),
			new Tuple<string, Func<IList<double>, double>, string>("Standard error", PerseusPluginUtils.StandardError,
				""),
			new Tuple<string, Func<IList<double>, double>, string>("Coefficient of variation",
				ArrayUtils.CoefficientOfVariation,
				"The coefficient of variation is defined as the standard deviation divided by the mean."),
			new Tuple<string, Func<IList<double>, double>, string>("Median absolute deviation (MAD)",
				ArrayUtils.MedianAbsoluteDeviation,
				"This is the median of the absolute values of the difference of each value to the median."),
			new Tuple<string, Func<IList<double>, double>, string>("Full width half maximum (FWHM)",
				ArrayUtils.FullWidthHalfMaximum,
				"Width of the histogram at half of the maximum value."),
			...
			new Tuple<string, Func<IList<double>, double>, string>("Inter-quartile range",
				ArrayUtils.InterQuartileRange, ""),
		...
		public string Url
			=> "https://cox-labs.github.io/coxdocs/summarystatisticsrows.html";
		...
		public void ProcessData(IMatrixData mdata, Parameters param, ref IMatrixData[] supplTables,
			...
			for (int i = 0; i < include.Length; i++){
				if (include[i]){
					...
					if (groups){
						for (int k = 0; k < groupNames.Length; k++){
							mdata.AddNumericColumn(procs[i].Item1 + " " + groupNames[k], procs[i].Item3,
								columnsG[i][k]);
						}
					}
				}
