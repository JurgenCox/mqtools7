...
namespace PerseusPluginLib.Load{
	public class UnstructuredTxtUpload : IMatrixUpload{
		...
		private static readonly (char separator, string name)[] Separators =
			{('\t', "Tab"), (',', "Comma"), (' ', "Space")};
		public Parameters GetParameters(ref string errString){
			return
				new Parameters(new FileParam("File"){
					...
					SubParamsTrue = new Parameters(new SingleChoiceParam("Separator")
						{Values = Separators.Select(sep => sep.name).ToArray()})
				});
		}
		...
		private static void LoadNoSplit(IMatrixData mdata, string filename){
			...
			mdata.SetAnnotationColumns(new List<string>(new[]{"All data"}),
				new List<string>(new[]{"Complete file in one text column."}),
				new List<string[]>(new[]{lines.ToArray()}), new List<string>(), new List<string>(),
				new List<string[][]>(),
		...
		private static void LoadSplit(IMatrixData mdata, string filename, char separator){
			...
			mdata.SetAnnotationColumns(new List<string>(colNames), new List<string>(colNames), new List<string[]>(cols),
				new List<string>(),
