...
namespace PerseusPluginLib.AnnotCols.AnnotationProvider{
	public static class AnnotationProviderExtensions{
		...
		/// <returns></returns>
		public static (string source, string id, string[] names)[] TextSources(
			this IAnnotationProvider annotationProvider){
		...
		/// <returns></returns>
		public static string[] MapToBaseIdentifiers(this IAnnotationProvider annotationProvider, string[] identifiers,
			int sourceIndex, int textAnnotationIndex){
			var selection = new[]{0};
		...
		/// <returns></returns>
		public static IEnumerable<string[][]> OuterLeftJoin(string[] identifiers,
			IEnumerable<(string fromId, string[][] toIds)> mapping, int numberOfMappings){
			var split = new[]{';'};
			...
			var outerLeftJoin = identifiers
				...
				.SelectMany((ids, row) =>
					ids.Split(split, StringSplitOptions.RemoveEmptyEntries).DefaultIfEmpty(emptyId)
						.Select(id => (id: id.Trim().ToLower(), row: row)))
				// join with mapping
				.GroupJoin(mapping, idRow => idRow.id, map => map.fromId,
					(idRow, maps) => {
						...
						var toIdsMaps = maps.DefaultIfEmpty((fromId: idRow.id, toIds: noMatch))
							.Select(map => map.toIds);
						var aggregators = Enumerable.Range(0, numberOfMappings).Select(_ => new HashSet<string>())
							.ToArray();
						...
						var toIds = aggregators
							.Select(idSet => idSet.Select(id => id.Trim()).OrderBy(id => id).ToArray()).ToArray();
				...
				.GroupBy(grp => grp.row, grp => grp.toIds, (row, allIds) => {
					...
					var toIds = aggregator.Select(idSet => idSet.Select(id => id.Trim()).OrderBy(id => id).ToArray())
						.ToArray();
		...
		/// <returns></returns>
		public static ((string name, string[] values)[] text, (string name, string[][] values)[] category, (string name,
			double[] values)[] numeric)
			ReadAnnotations(this IAnnotationProvider annotationProvider, string[] identifiers, int sourceIndex,
				int[] annotationIndices){
			...
			var indexedAnnotations = annotationProvider.Sources[sourceIndex].annotation
				.Select((annotation, i) => (annotation: annotation, i: i));
			var annotations = annotationIndices.Join(indexedAnnotations, i => i, iAnnotation => iAnnotation.i,
				(i, iAnnotation) => iAnnotation.annotation).ToArray();
			...
			for (int i = 0; i < annotationIndices.Length; i++){
				...
				switch (type){
					...
					case AnnotType.Numerical:
						numeric.Add((name,
							columns[i].Select(s => Parser.Double(s.DefaultIfEmpty("NaN").Single())).ToArray()));
						break;
