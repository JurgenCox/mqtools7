...
namespace PerseusPluginLib.AnnotCols.AnnotationProvider{
	...
	public static class AnnotationFormat{
		...
		/// <returns></returns>
		public static IEnumerable<(string fromId, string[][] toIds)> ReadMappings(StreamReader reader, int fromColumn,
			int[] toColumns){
			char[] semicolon ={';'};
			var mapping = ReadLines(reader).Skip(2).SelectMany(line => {
				...
				var toIdentifiers = toColumns
					.Select(toColumn => entries[toColumn].Split(semicolon, StringSplitOptions.RemoveEmptyEntries))
					.ToArray();
				return fromIdentifiers.Select(fromId =>
					(fromId: fromId.Trim().ToLower(), toIdentifiers: toIdentifiers));
			});
		...
		/// <param name="toColumns"></param>
		/// <returns></returns>
		public static IEnumerable<(string fromId, string[][] toIds)> ReadMappings(string file, int fromColumn,
			int[] toColumns){
		...
		public static void WriteMapping((string name, string[] values) baseIdentifiers,
			(string name, string[] values)[] text,
		...
		public static void WriteMapping((string name, string[] values) baseIdentifiers,
			...
			for (int i = 0; i < baseIdentifiers.values.Length; i++){
				...
				var row = new[]{baseIdentifiers.values[captureI]}
					.Concat(category.Select(annot =>
						string.Join(";", annot.values[captureI].Where(values => !string.IsNullOrEmpty(values)))))
