...
namespace PerseusPluginLib.AnnotCols{
	public class AddAnnotationToMatrix : IMatrixProcessing{
		...
		public string Url
			=>
                "https://cox-labs.github.io/coxdocs/addannotationtomatrix.html";
		...
		/// <returns></returns>
		public static Parameters CreateParameters(IList<string> colChoice, IAnnotationProvider annotationProvider,
			ref string errorString){
			if (annotationProvider.BadSources.Any()){
				errorString =
					$"Could not load annotations from file(s): {string.Join(", ", annotationProvider.BadSources)}";
			}
			int selFile = 0;
				...
				subParams[i] =
					new Parameters(
						...
						new MultiChoiceParam("Annotations to be added")
							{Values = source.annotation.Select(annot => annot.name).ToArray()});
			}
		...
		/// <returns></returns>
		private static string[] GetBaseIds(IDataWithAnnotationColumns mdata, IAnnotationProvider annotationProvider,
			Parameters para){
		...
		public static void ProcessData(IDataWithAnnotationColumns mdata, IAnnotationProvider annotationProvider,
			Parameters para, ProcessInfo processInfo){
			string[] baseIds = GetBaseIds(mdata, annotationProvider, para);
			bool success = ProcessDataAddAnnotation(mdata.RowCount, annotationProvider, para, baseIds, processInfo,
				out string[] name,
				out int[] catColInds,
		...
		public static bool ProcessDataAddAnnotation(int nrows, IAnnotationProvider annotationProvider, Parameters para,
			string[] baseIds, ProcessInfo processInfo,
			out string[] name, out int[] catColInds, out int[] textColInds, out int[] numColInds,
			out string[][][] catCols,
			...
			int ind = spd.Value;
			Parameters param = spd.GetSubParameters();
			var types = annotationProvider.Sources
				.Select(s => s.annotation.Select(annotation => annotation.type).ToArray()).ToArray();
			var type = types[ind];
			var names = annotationProvider.Sources
				.Select(s => s.annotation.Select(annotation => annotation.name).ToArray()).ToArray();
			name = names[ind];
		...
		public static Dictionary<string, string[]> ReadMapping(ICollection<string> allIds, string file,
			IList<int> selection){
