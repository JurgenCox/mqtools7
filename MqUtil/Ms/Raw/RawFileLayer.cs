...
namespace MqUtil.Ms.Raw{
	...
	public class RawFileLayer : IDisposable{
		...
		public double[] FaimsVoltages{ get; set; }
		...
		protected internal RawFileLayer(BinaryReader reader, RawFile rawFile, bool positive){
			...
			Ms2WindowGroup = FileUtils.ReadInt32Array(reader);
		...
		public double EndTimeMs1 =>
			Ms1Rt.Length < 1 ? 1 : 1.5f * GetMs1Time(Ms1Count - 1) - 0.5f * GetMs1Time(Ms1Count - 2);
		...
		public void GetMs2SpectrumArray(int index, bool readCentroids, out double[] masses, out float[] intensities){
			rawFile.GetSpectrum(Ms2ScanNumbers[index], readCentroids, out masses, out intensities, false);
		}
		...
		public void GetMs3SpectrumArray(int index, bool readCentroids, out double[] masses, out float[] intensities){
			rawFile.GetSpectrum(Ms3ScanNumbers[index], readCentroids, out masses, out intensities, false);
		}
		public void GetMs1SpectrumArray(int index, bool readCentroids, out double[] masses, out float[] intensities){
			...
			rawFile.GetSpectrum(Ms1ScanNumbers[index], readCentroids, out masses, out intensities, true);
		}
		public double[] Index2K0Ms1(int index, double[] imsInds){
			return index >= Ms1ScanNumbers.Length ? null : rawFile.Index2K0(Ms1ScanNumbers[index], imsInds);
		}
		...
		public IntSpectrum[] GetMs1Spectrum(int indexMin, int indexMax, int[] imsIndexMin, int[] imsIndexMax,
			bool readCentroids){
			if (indexMax >= Ms1ScanNumbers.Length || indexMin < 0){
				return null;
			}
			return rawFile.GetSpectrum(Ms1ScanNumbers[indexMin], Ms1ScanNumbers[indexMax], imsIndexMin, imsIndexMax,
				readCentroids);
		}
		...
		public IntSpectrum[] GetMs2Spectrum(int indexMin, int indexMax, int[] imsIndexMin, int[] imsIndexMax,
			bool readCentroids){
			if (indexMax >= Ms2ScanNumbers.Length || indexMin < 0){
				return null;
			}
			return rawFile.GetSpectrum(Ms2ScanNumbers[indexMin], Ms2ScanNumbers[indexMax], imsIndexMin, imsIndexMax,
				readCentroids);
		}
		...
		public static double[][]
			CalcBinMinMaxInSpectrum(double minX, double maxX, int xCount, double[] sx, double[] sy){
			if (sx == null){
				return null;
			}
		...
		public Spectrum GetMs1Spectrum(int index, bool readCentroids){
			GetMs1SpectrumArray(index, readCentroids, out double[] masses, out float[] intensities);
		...
		public Spectrum GetMs2Spectrum(int index, bool readCentroids){
			GetMs2SpectrumArray(index, readCentroids, out double[] masses, out float[] intensities);
			return new Spectrum(masses, intensities);
		}
		...
		public Spectrum GetMs3Spectrum(int index, bool readCentroids){
			GetMs3SpectrumArray(index, readCentroids, out double[] masses, out float[] intensities);
			return new Spectrum(masses, intensities);
		}
		...
		public void GetMs1DiagnosticData(int index, out double intenseCompFactor, out double emIntenseComp,
			...
			intenseCompFactor = index >= 0 && index < Ms1IntenseCompFactor.Length
				? Ms1IntenseCompFactor[index]
				: double.NaN;
		...
		public void GetMs2DiagnosticData(int index, out double intenseCompFactor, out double emIntenseComp,
			out double rawOvFtT, out double agcFill){
			intenseCompFactor = index >= 0 && index < Ms2IntenseCompFactor.Length
				? Ms2IntenseCompFactor[index]
				: double.NaN;
		...
		internal void Write(BinaryWriter writer){
			...
			FileUtils.Write(Ms2WindowGroup, writer);
		...
		private void SetMs1Data(Ms1Lists ms1Lists){
			...
			if (!RawFileUtils.IsMonotoneIncreasing(Ms1Rt, out int startPos)){
				Ms1ScanNumbers = Ms1ScanNumbers.SubArrayFrom(startPos);
				Ms1Rt = Ms1Rt.SubArrayFrom(startPos);
				Ms1IonInjectionTimes = Ms1IonInjectionTimes.SubArrayFrom(startPos);
				Ms1BasepeakIntensities = Ms1BasepeakIntensities.SubArrayFrom(startPos);
				Ms1ElapsedTimes = Ms1ElapsedTimes.SubArrayFrom(startPos);
				Ms1Tic = Ms1Tic.SubArrayFrom(startPos);
				ms1HasCentroid = ms1HasCentroid.SubArrayFrom(startPos);
				ms1HasProfile = ms1HasProfile.SubArrayFrom(startPos);
				ms1Analyzer = ms1Analyzer.SubArrayFrom(startPos);
				ms1MinMass = ms1MinMass.SubArrayFrom(startPos);
				ms1MaxMass = ms1MaxMass.SubArrayFrom(startPos);
				Ms1Resolution = Ms1Resolution.SubArrayFrom(startPos);
				Ms1IntenseCompFactor = Ms1IntenseCompFactor.SubArrayFrom(startPos);
				Ms1EmIntenseComp = Ms1EmIntenseComp.SubArrayFrom(startPos);
				Ms1RawOvFtT = Ms1RawOvFtT.SubArrayFrom(startPos);
				Ms1AgcFill = Ms1AgcFill.SubArray(startPos);
			}
		}
		...
		private void SetMs2Data(Ms2Lists ms2Lists){
			...
			Ms2WindowGroup = ms2Lists.windowGroupList.ToArray();
		...
		public void GetDiaRanges(out double[] mzMin, out double[] mzMax){
			int[][] inds =
				RawFile.GetDiaMs2Indices(Ms2IsolationMzMin, Ms2PrevMs1Index, Ms1Count, Ms2ScanNumbers.Length - 1);
		...
		public int GetDiaNumMs1Scans(){
			...
			int[][] x = RawFile.GetDiaMs2Indices(Ms2IsolationMzMin, Ms2PrevMs1Index, Ms1Count,
				Ms2ScanNumbers.Length - 1);
			if (x.Length == 0){
				return 1;
			}
			int len = x[0].Length;
