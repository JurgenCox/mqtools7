	...
	public class Molecule{
		...
		private static string ProcessFirst(string formula, IList<int> counts){
			for (int i = ChemElements.Elements.Length - 1; i >= 0; i--){
				...
				foreach (string altSymbol in ChemElements.Elements[i].AltSymbols){
					if (formula.StartsWith(altSymbol)){
						return Calc(formula, counts, altSymbol, i);
					}
				}
			}
		...
		private static string Calc(string formula, IList<int> counts, string symbol, int i){
			...
			while (index < formula.Length &&
			       ((formula[index] >= '0' && formula[index] <= '9') || formula[index] == '-')){
				index++;
			}
			int amount;
			if (index == 0){
		...
		private double[][] GetIsotopeSpectrum(double sigma, int pointsPerSigma, double massPrecision){
			...
			for (int i = 0; i < n; i++){
				...
				for (int j = 0; j < masses.Length; j++){
					newWeights[i] +=
						weights[j] * Math.Exp(-(mass - masses[j]) * (mass - masses[j]) / 2 / sigma / sigma);
				}
			}
		...
		public static void ConvoluteWithoutErrors(double[] masses1, double[] weights1, double[] masses2,
			double[] weights2, double massCutoff, double weightCutoff, out double[] masses, out double[] weights){
		...
		protected bool Equals(Molecule other){
			return ArrayUtils.EqualArrays(AtomType, other.AtomType) &&
			       ArrayUtils.EqualArrays(AtomCount, other.AtomCount);
		}
