...
namespace MqUtil.Parse{
	public static class PhosphoSitePlusParser{
		...
		public static void ParseKnownMods(string filename, out string[] seqWins, out string[] accs,
			out string[] pubmedLtp,
			out string[] pubmedMs2, out string[] cstMs2, out string[] species){
		...
		public static void ParseRegulatorySites(out string[] seqWins, out string[] accs, out string[] function,
			...
			string[] notes1;
			string[] species1;
			ParseRegulatorySites(out seqWins1, out accs1, out function1, out process1, out protInteract1,
				out otherInteract1,
		...
		public static void ParseRegulatorySites(out string[] seqWins, out string[] accs, out string[] function,
			...
			ParseRegulatorySites(filename, out seqWins, out accs, out function, out process, out protInteract,
				out otherInteract,
		...
		private static readonly string[] phosphositeFilenames ={
			...
			"O-GlcNAc_site_dataset", "O-GalNAc_site_dataset", "Phosphorylation_site_dataset",
		...
		private static void ParsePhosphoSite(TextReader reader, IDictionary<string, string> map){
			...
			while ((line = reader.ReadLine()) != null){
				...
				string type = w[4];
				string residue = w[5];
				int position = Parser.Int(residue.Substring(1));
