using MqApi.Drawing.Image.Formats.Gif.Sections;
namespace MqApi.Drawing.Image.Formats.Gif{
	internal class GifDecoderCore{
		...
		private void ReadLogicalScreenDescriptor(){
			...
			if (logicalScreenDescriptor.Width > decodedImage.MaxWidth ||
			    logicalScreenDescriptor.Height > decodedImage.MaxHeight){
		...
		private void ReadFrameColors(byte[] indices, byte[] colorTable, GifImageDescriptor descriptor){
			...
			if (graphicsControlExtension != null &&
			    graphicsControlExtension.DisposalMethod == DisposalMethod.RestoreToPrevious){
			...
			for (int y = descriptor.Top; y < descriptor.Top + descriptor.Height; y++){
				...
				for (int x = descriptor.Left; x < descriptor.Left + descriptor.Width; x++){
					...
					if (graphicsControlExtension == null || graphicsControlExtension.TransparencyFlag == false ||
						...
						Color2 pixel = Color2.FromArgb(colorTable[indexOffset], colorTable[indexOffset + 1],
							colorTable[indexOffset + 2]);
