...
using MqApi.Drawing.Image.Quantizers;
using MqApi.Num;
using MqApi.Util.Endian;
namespace MqApi.Drawing.Image.Formats.Gif{
	internal sealed class GifEncoderCore{
		...
		public void Encode(ImageBase imageBase, Stream stream){
			...
			Quality = quality > 0 ? NumUtil.Clamp(quality, 1, 256) : 256;
		...
		private void WriteLogicalScreenDescriptor(Image2 image, EndianBinaryWriter writer, int tranparencyIndex){
			...
			field.SetBit(0,
				descriptor.GlobalColorTableFlag); // 1   : Global color table flag = 1 || 0 (GCT used/ not used)
		...
		private void WriteGraphicalControlExtension(ImageBase image, EndianBinaryWriter writer,
			...
			DisposalMethod disposalMethod =
				hasTransparent ? DisposalMethod.RestoreToBackground : DisposalMethod.Unspecified;
