...
using MqApi.Drawing.Image.Formats.Png.Zlib;
...
namespace MqApi.Drawing.Image.Formats.Png{
	internal sealed class PngEncoderCore{
		...
		public void Encode(ImageBase image, Stream stream){
			...
			Quality = quality > 0 ? NumUtil.Clamp(quality, 1, int.MaxValue) : int.MaxValue;
			bitDepth = Quality <= 256
				? (byte) NumUtil.Clamp(GifEncoderCore.GetBitsNeededForColorDepth(Quality), 1, 8)
				: (byte) 8;
			if (bitDepth == 3){
				bitDepth = 4;
