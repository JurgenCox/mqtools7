			...
			try{
				...
				if (infoHeader.Width > image.MaxWidth || infoHeader.Height > image.MaxHeight){
					throw new ArgumentOutOfRangeException(
						$"The input bitmap '{infoHeader.Width}x{infoHeader.Height}' is " +
				...
				switch (infoHeader.Compression){
					case BmpCompression.Rgb:
						...
						if (infoHeader.BitsPerPixel == 32){
							...
							ReadRgbPalette(imageData, palette, infoHeader.Width, infoHeader.Height,
								infoHeader.BitsPerPixel, inverted);
						}
						break;
					default:
			...
			Parallel.For(0, height, Bootstrapper.instance.ParallelOptions, y => {
				...
				for (int x = 0; x < arrayWidth; x++){
					...
					for (int shift = 0; shift < ppb && (colOffset + shift) < width; shift++){
						...
						Color2 packed = Color2.FromArgb(colors[colorIndex + 2], colors[colorIndex + 1],
							colors[colorIndex]);
