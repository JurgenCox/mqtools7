...
namespace MqApi.Calc.Util{
	internal static class FuncUtils{
		...
		private static readonly Dictionary<char, char> brackets = new Dictionary<char, char>
			{{'(', ')'},{'{', '}'},{'[', ']'}};
		internal static TreeNode ParseString(string text, IList<string> realVariableNames,
			IList<string> intVariableNames,
			ref string errMsg){
		...
		private static TreeNode ParseImpl(string text, IList<string> realVariableNames, IList<string> intVariableNames,
			...
			switch (opPos.Length){
				case 0:
					...
					if (pos.Length == 1){
						if (funcPrefix[0] != null){
							return ParseFunctionCall(text, funcPrefix[0], realVariableNames, intVariableNames,
								ref errMsg);
						}
				...
				default:
					...
					if (opPos1.Length > 0){
						return ParseOperator(text, opPos1[opPos1.Length - 1], realVariableNames, intVariableNames,
							ref errMsg);
					}
					return ParseOperator(text,
						opPos2.Length > 0 ? opPos2[opPos2.Length - 1] : opPos3[opPos3.Length - 1],
		...
		private static TreeNode ParseBrackets(string text, IList<string> realVariableNames,
			IList<string> intVariableNames,
			ref string errMsg){
