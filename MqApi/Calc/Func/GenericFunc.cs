...
namespace MqApi.Calc.Func{
	...
	internal abstract class GenericFunc{
		...
		internal static readonly TreeNode negInfinity = new TreeNode
			{Func = negativeInfinity, arguments = new TreeNode[0]};
		internal static readonly TreeNode posInfinity = new TreeNode
			{Func = positiveInfinity, arguments = new TreeNode[0]};
		...
		internal static readonly Func1[] allFunc1 = new[]{
			...
			new Func1Abs(), new Func1Ceiling(), dirac, exp, new Func1Floor(), heavyside, inverse, log, log10, log2,
			minus,
		...
		internal static readonly Dictionary<string, List<GenericFunc>> funcMap =
			new Dictionary<string, List<GenericFunc>>();
		...
		internal static TreeNode IntVar(int index){
			return new TreeNode
				{Func = new Variable{type = ReturnType.Integer, id = index}, arguments = new TreeNode[0]};
		}
		static GenericFunc(){
			AddRange(allFunc1);
			AddRange(allFunc2);
